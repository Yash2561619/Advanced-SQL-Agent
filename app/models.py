...
class SQLCorrectionResult(BaseModel):
    analysis: str = Field(
        ...
        description="Analysis of why the current SQL query is not producing relevant results"
    )
    identified_issues: Union[List[str], str] = Field(
        default="No issues identified",
        description="List of specific issues or text description of issues identified in the current SQL query"
    )
    ...
    @validator("identified_issues", pre=True)
    def normalize_identified_issues(cls, v):
        if isinstance(v, list):
            return "; ".join(str(item) for item in v)
        return str(v) if v is not None else "No issues identified"
...
class AgentState(TypedDict):
    ...
    visualization_explanation: Optional[str]