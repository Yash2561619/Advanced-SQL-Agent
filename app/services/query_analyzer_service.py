...
def query_analyzer_table_selector(state: AgentState) -> AgentState:
    ...
    llm = get_llm(Config.LLM_PROVIDER, Config.LLM_MODEL)  # gpt-3.5-turbo, GPT-4o-mini
    # llm = get_llm("groq", "gemma2-9b-it")

    relevant_memories = "\n".join(
        [f"Memory {i+1}: {memory.page_content}" for i, memory in enumerate(state.get("relevant_memories", []))]
    )
