...
class TestConfig(Config):
    TESTING = True
    SQLALCHEMY_DATABASE_URI = 'sqlite:///file:test_db?mode=memory&cache=shared&uri=true'
    SECRET_KEY = 'test-secret-key'

    # Temporary directories for testing isolation
    SESSION_FILE_DIR = tempfile.mkdtemp()
    CHROMA_PERSIST_DIRECTORY = tempfile.mkdtemp()

    # Modern flask_session config to avoid deprecation warnings
...
class TestAPI(unittest.TestCase):
    ...
    def test_analyze_query_endpoint(self, mock_create_analysis_graph):
        ...
        # Verify the actual response payload structure from /analyze
